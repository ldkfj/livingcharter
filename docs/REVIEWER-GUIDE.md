...
3. Open **Requests**: four completed requests covering every outcome — #1 APPROVE (the ruling quotes the real $469 ticket price it read from the PyCon page), #2 DENY with an upheld appeal, #3 DENY under the old article 4, #4 PARTIAL 0.03/0.05 GEN under the amended article 4. Each ruling card shows the decision, amount, cited articles, and the validators' reasoning.
4. Open **Precedents**: the append-only ruling log (5 entries at release, one marked as an appeal ruling).
